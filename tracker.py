...
class Tracker:
    ...
    def match(self, gray_cut):
        ...
        # в случае первого вхождения присваиваем множеству сохраняемых id и дескрипторов первое значение
        ...
        # сравнение сохраненных ключевых точек с новыми
        # ищем наибольшее количество совпадений с имеющимся набором, в self.id_deckriptor сохраняем id под которым был данный набор точек
        # если действующий (основной) счетчик id больше найденного, значи этот человек уже встречался и был на кадре, то есть
        # происходит откат основного id до найденного значения
        try:
            mach = []
